...
| `src/hooks/useWindowSizePersistence.ts` | 通常時の論理サイズ監視、終了前保存、起動時の復元と作業領域への補正                     |
| `src/domain/windowSize.ts`              | 既定サイズと、保存サイズを表示可能範囲へ収める純粋計算                                 |
...
ウィンドウサイズも同じAppState更新キューを通し、別の保存先や競合規則を持たない。リサイズ通知は短時間まとめて保存し、通常終了要求では未保存の最新サイズを先にキューへ加えて完了を待つ。Tauri APIへの依存はフックに閉じ、寸法補正は純粋関数として単体テストする。
