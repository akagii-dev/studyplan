...
承認済みPlanは生成時の有効な実績合計、将来セッションの問題数・終了時刻、未配置量をprogressBaselineへ保持する。進捗の追加・訂正・取消では、この不変な基準から全結果を再計算するため、保存再送や再起動で同じ実績を二重控除しない。前倒し反映は同じ教材・同じ周回だけを対象とし、実績の日付、別教材、別周回、固定予定、他の予定時刻を変更しない。新しい計画を生成した時点で現在実績を新しい基準へ取り込む。
