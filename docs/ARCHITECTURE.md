...
- `pwa`モードだけが`pwaStore.ts`のIndexedDBを使用する。画面、計画計算、保存キュー、リビジョン競合と失敗時の再読込はデスクトップと共通。SQLite・公開デモのlocalStorageから自動移行しない。
- サーバーは`dist-pwa`の検証済み静的ファイルを配るだけ。学習データを受け取るAPIを持たない。バックアップ・復元前の退避・書き出しは端末内で処理する。
- `scripts/build-pwa.mjs`が全JS（遅延読込のバックアップ画面を含む）・CSS・HTML・アイコン・manifestを列挙し、SHA-256を確定する。Service Workerはすべて検証・保存できて初めて有効になる。画面上のオフライン準備は実際のキャッシュを照合して判定する。
- Workerは`/studyplan-pwa/`のみ制御する。既存タブがある間は新Workerを待機させ、全タブを閉じた次回に更新。旧アプリ用Cache Storageだけを整理し、IndexedDBや他アプリのキャッシュは削除しない。
- `PwaStatus`の配信元接続チェックとAppShellの保存状態は別。ネットワーク断をIndexedDB保存失敗に変換しない。LAN HTTPではWorkerが使えないことを明示し、オフライン準備完了にしない。
- 初回HTTPS接続とキャッシュ完了がオフライン利用の前提。URLのプロトコル・ホスト・ポート変更は別の保存領域になる。iOSのSafariとホーム画面アプリも保存領域を共有すると仮定しない。移行は既存形式のバックアップ経由。
- ブラウザー終了前の未保存警告は補助であり、iOSの強制終了を阻止できない。画面の「保存済み」を確認して閉じる。OSの容量整理やユーザーによるブラウザーデータ削除に備えて外部ファイルへバックアップする。

配信・停止・HTTPSへの切替は[配布手順](RELEASING.md)。PWAの隔離ブラウザー検証は`pnpm verify:pwa`。LAN HTTP、localhostのsecure context、信頼されたHTTPSとiOS実機の検証結果を区別して記録する。
