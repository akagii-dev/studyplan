...
import { ProgressReceipt, summarizePlanChanges } from '../domain/progressReceipt';
...
export function receiptOutcome(receipt: ProgressReceipt) {
  if (receipt.status === 'unplaced') {
    ...
    const quantity =
      receipt.shortfalls.length === 1
        ? `${receipt.shortfalls[0].count}問`
        : `${receipt.shortfalls.length}項目`;
    return `未配置 ${quantity}・${duration(minutes)}`;
  }
  if (receipt.status === 'applied') {
    const summary = summarizePlanChanges(receipt.changes);
    return `明日以降を調整 · ${[
      summary.quantity ? `数量 ${summary.quantity}件` : '',
      summary.placement ? `配置のみ ${summary.placement}件` : '',
    ]
      .filter(Boolean)
      .join('・')}`;
  }
  return statusNames[receipt.status];
}
...
export function ProgressReceiptView({
  state,
  receipt,
}: {
  state: AppState;
  ...
  const materialName = (id: string) =>
    state.settings.materials.find((item) => item.id === id)?.name ?? id;
  ...
  return (
    <div className="progress-receipt">
      ...
      {receipt.changes.length > 0 && (
        <ul>
          {receipt.changes.map((item, index) => (
            <li
              key={`${item.date}/${item.materialId}/${item.round}/${item.kind}/${item.fixed}/${index}`}
            >
              ...
              {item.kind === 'review'
                ? '復習'
                : `${materialName(item.materialId)} · ${item.round + 1}周目`}{' '}
              {item.kind === 'review'
        ...
        <ul>
          {receipt.shortfalls.map((item, index) => (
            <li key={`${item.materialId}/${item.round}/${index}`}>
              未配置 {materialName(item.materialId)} · {item.round + 1}周目 {item.count}問・
      ...
      {receipt.detail && (
        <small className={receipt.status === 'failed' ? 'error' : ''}>{receipt.detail}</small>
      )}
    </div>
  );
}
